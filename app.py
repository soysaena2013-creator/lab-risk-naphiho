...
with tab1:
    ...
    # [ส่วนที่ 1] ข้อมูลทั่วไป (อยู่นอกฟอร์มเพื่อให้ระบบเกิดการตอบสนองทันที)
    ...
    # [ส่วนที่ 2] ตัวเลือกย่อยขยับออกมานอกฟอร์ม เพื่อเปิดระบบเปลี่ยนตามคลิกทันที (Dynamic Form)
    ...
    # [ส่วนที่ 3] ใช้ปุ่มกดส่งของ Streamlit แบบเดี่ยวๆ เพื่อทำการเซฟข้อมูลลงฐานข้อมูล
...
with tab2:
    ...
    if st.session_state.raw_data.empty:
        ...
        st.subheader("✍️ ส่วนลงคะแนนเข้าพิกัด Risk Matrix")
        ...
        l_score = st.slider("คะแนนความถี่ประจำเดือน (Likelihood Score: 1-5)", 1, 5, 3)
        s_score = st.slider("คะแนนความรุนแรงของปัญหาประจำเดือน (Severity Score: 1-4)", 1, 4, 2)

        eval_submit = st.button("💾 บันทึกผลประเมินประจำเดือน")
        ...
        if eval_submit:
            ...
            st.success(f"ประเมินเรียบร้อย! Risk Score = {l_score * s_score} คะแนน")
        ...
        if df_filtered.empty:
            ...
            with col_g1:
                ...
                for score in df_sorted['Risk_Score']:
                    if score >= 12: colors.append('crimson')
                    elif score >= 8: colors.append('darkorange')
                    elif score >= 4: colors.append('gold')
