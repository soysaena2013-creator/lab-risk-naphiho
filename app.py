...
def calculate_likelihood(count):
    if count == 1:
        ...
        return 3  # เกิดปานกลาง
    elif count <= 6:
        return 4  # เกิดบ่อย
    else:
        return 5  # เกิดบ่อยมาก (ทุกวัน/เกือบทุกวัน)
...
with tab3:
    ...
    if st.session_state.monthly_summary.empty:
        ...
        if df_filtered.empty:
            ...
            with col_g1:
                st.subheader("📌 อันดับความเสี่ยงเร่งด่วนประจำเดือน (ตามความถี่และความรุนแรงจริง)")
            ...
            st.markdown("---")
            st.subheader("🧱 5x4 Interactive Risk Matrix (จัดพิกัดตามสถิติจริง)")
